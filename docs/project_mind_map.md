...
mindmap
  root((Шерстяной Синдикат: 9 Жизней))
    ...
    UX
      ...
      "обычные фразы: мой котёнок, досье, инвентарь, кузница, магазин, ежедневки, работа 5, подвал 5"
    ...
    Код
      ...
      services
        ...
        crafting_service["services/crafting.py: рецепты, магазин, экипировка, расходники, бонусы"]
...
| `/shop` / `/buy [название]` | расходники | рыбов |
...
| `buy_inventory_item` | купить расходник; не списывает рыбов, если стек 9999 заполнен |
...
| Реплики полного стека магазина | `SHOP_STACK_FULL_TEXTS` в `handlers/shop.py` |
...
4. Учесть исход ковки: успех, провал, крит-успех или крит-провал.
5. Проверить, что списались материалы и оплата кузнецу рыбами.
6. Повторно сковать тот же предмет с износом и проверить, что прочность восстановилась.
7. Сковать следующий тир и проверить, что предмет улучшился вместо появления дубля.
8. В `🎒 Инвентарь` нажать кнопку экипировки.
9. Для классового оружия проверить, что надеть можно только оружие своего класса.
10. Проверить `🧥 Экипировка`.

### Магазин

1. Открыть `🏪 Магазин` или написать `магазин`.
2. Купить расходник кнопкой или командой `/buy Валерьянка`.
3. Проверить, что списались рыбовы.
4. В `🎒 Инвентарь` использовать расходник.
