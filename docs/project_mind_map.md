  ...
  root((Шерстяной Синдикат: 9 Жизней))
    ...
    Ресурсы
      ...
      Кузница
        ...
        "крит успех расходников даёт бонусный экземпляр"
...
Практичный следующий шаг после доработки кузницы: начать с `Магазина расходников`, потому что он небольшой по объёму, использует уже готовые `inventory`/`buffs` и добавляет новый способ тратить рыбов.
...
5. В `🎒 Инвентарь` нажать кнопку расходника или экипировки.
6. Для классового оружия проверить, что надеть можно только оружие своего класса.
7. Проверить `🧥 Экипировка`.
